...
public class FTPUtil {
     ...
     * @throws Exception
     */
    public static void uploadFiles(FTPClient ftpClient, String baseUploadPath,File uploadFile) {
        /**如果 FTP 连接已经关闭，或者连接无效，则直接返回*/
        if (!ftpClient.isConnected() || !ftpClient.isAvailable()) {
            ...
            if (uploadFile.isDirectory()) {
                ...
                for (int i = 0; i < listFiles.length; i++) {
                    ...
                    if (loopFile.isDirectory()) {
                        /**如果有子目录，则迭代调用方法进行上传*/
                        uploadFiles(ftpClient, baseUploadPath,loopFile);
                        ...
                        String loopUploadPath = baseUploadPath+'\\'+loopFile.getName();
                        ftpClient.storeFile(loopUploadPath, input);
                        input.close();
                        System.out.println(">>>>>文件上传成功****" + loopUploadPath);
                    }
                }
            } else {
                /**如果被上传的是文件时*/
                 ...
                 * */
                String uploadPath = baseUploadPath+'\\'+uploadFile.getName();
                System.out.println(uploadPath);
                ftpClient.storeFile(uploadPath, input);
                input.close();
...
/*        // ============================= 单文件下载
        ...
        // ============================ 文件夹下载
        List<String> relativePathList = new ArrayList<>();
        relativePathList = loopServerPath(ftpClient, "\\images", relativePathList);
