...
public class TeacherServiceImpl implements TeacherService {
    ...
    @Cacheable(value = "FacultyTeacherAll", key = "#jsonObject.getLongValue(\"unitId\")+'listFacultyTeacherAll'")
    @Override
    ...
    @Override
    public JSONObject getRecommendTeacher(JSONObject jsonObject) {
        List<JSONObject> list = teacherDao.getRecommendTeacher(jsonObject);
    ...
    public JSONObject listTeacherRecommend(JSONObject jsonObject) {
        ...
        if(listScholatTemp.size()!=0 && listScholatTemp.get(0)!=null){
            ...
            // 这里推荐10个人就可以了
            for(int i=1;i<11;i++){
                JSONObject jsonObject2 = teacherDao.getLocalInfoByID(str_arr[i]);
                res.add(jsonObject2);
            }
            list.addAll(res);
        }
        if(list.size()<10){
            int left = 10-list.size();
            // 没有就去看下本地推荐
            for(int i = 0;i<26;i++){
                jsonObject.put("letter",c);
                List<JSONObject> listTemp = teacherDao.listTeacherByletter(jsonObject);
                list.addAll(listTemp);
                if(list.size()>=left)break;
                else {
                    c=(char)((byte)c+1);
                }
            }
