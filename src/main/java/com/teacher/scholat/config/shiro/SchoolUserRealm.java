 ...
 * @date:
 */
public class SchoolUserRealm extends AuthorizingRealm {
    ...
    protected AuthenticationInfo doGetAuthenticationInfo(AuthenticationToken authcToken) throws AuthenticationException {
        System.out.println("进到了SchoolUserRealm获取登录验证");
        String loginName = (String) authcToken.getPrincipal();
        // 获取用户密码
        String password = new String((char[]) authcToken.getCredentials());
        // 角色是学院用户
        String host = "school";
        // 去找数据库中login表的数据是否有该账户
...
//            jsonObject.put("msg", "用户已锁定，请联系管理员进行处理!");
        ...
        SecurityUtils.getSubject().getSession().setAttribute(Constants.SESSION_SCHOOL_USER_INFO, user);
